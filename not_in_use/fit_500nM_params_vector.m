...
% in 500 nM continous and intermittent treatments for vector (control
% cells)
%
%%%%%%%%%%%
...
no_updates_on = 1:5;%updates per day when the drug is on. 0:10 in manuscript.
no_updates_off = 1:5; %updates per day when the drug is off. 0:10 in manuscrip.
no_n = 4; %fineness of below parameters to test. 100 in manuscript, 10 here for computational speed
...
data_time = [1,7:7:28];
data_cont=cf(2:7,4:end);
data_int=cf(8:13,4:end);
...
%data_int=data_int.*(1-isoutlier(data_int)); 
...
% mean_int_data(t)=mean(tmp_vec_int);
% SEM_int(t)=std(tmp_vec_int)/sqrt(length(tmp_vec_int));
mean_cont_data=mean(data_cont);
SEM_cont=std(data_cont)/sqrt(length(data_cont));
mean_int_data=mean(data_int);
SEM_int=std(data_int)/sqrt(length(data_int));
...
remove = cost_list(:,6)  == 2;  % 2nd col <2 (i.e. 0 -not done- or 1 -not enough-)
cost_list(remove,:) = [] ;  % remove those rows

save('workspace_500nM_costs_vector.mat')
